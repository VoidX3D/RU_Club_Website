...
import TypewriterText from '@/components/secret-garden/TypewriterText'
import SkillBar from '@/components/secret-garden/SkillBar'
import ProjectCard from '@/components/secret-garden/ProjectCard'
...
const projects = [
  { name: 'RU Club Motherland', desc: 'Environmental club platform — React, TypeScript, Supabase, Tailwind CSS. Full-stack SPA with real-time DB, gallery, and admin panel.', url: SITE_URL, repo: 'https://github.com/RU-Club-Motherland', color: 'emerald', tags: ['React', 'TypeScript', 'Supabase', 'Tailwind'] },
...
const techIcons = [
  { name: 'React', emoji: '⚛️' }, { name: 'TypeScript', emoji: '📘' }, { name: 'Node.js', emoji: '🟢' },
  { name: 'Python', emoji: '🐍' }, { name: 'Git', emoji: '🔀' }, { name: 'Docker', emoji: '🐳' },
  { name: 'VS Code', emoji: '💻' }, { name: 'Supabase', emoji: '🔥' }, { name: 'Figma', emoji: '🎨' },
  { name: 'Vite', emoji: '⚡' },
]

const connectLinks = [
  { platform: 'GitHub', handle: '@VoidX3D', url: 'https://github.com/VoidX3D',
  ...
  { platform: 'Instagram', handle: '@sincerebhattarai', url: 'https://instagram.com/sincerebhattarai',
  ...
  { platform: 'Facebook', handle: 'RU Club Motherland', url: 'https://facebook.com/profile.php?id=61585206314774',
    icon: <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />, fill: true },
  { platform: 'Email', handle: 'sincerebhattarai01@gmail.com', url: 'mailto:sincerebhattarai01@gmail.com',
    icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></>, fill: false },
  { platform: 'RU Club GitHub', handle: 'RU-Club-Motherland', url: 'https://github.com/RU-Club-Motherland',
    icon: <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" />, fill: true },
  { platform: 'RU Club Website', handle: SITE_URL.replace('https://', ''), url: '/',
    icon: <><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>, fill: false },
  { platform: 'Personal Site', handle: 'ubuntu-sincere.vercel.app', url: 'https://ubuntu-sincere.vercel.app',
    icon: <><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>, fill: false },
]
          ...
          <section className="min-h-screen flex items-center justify-center px-4 py-20">
            <div className="max-w-3xl w-full text-center">
              <div className="relative inline-block mb-8">
                <div className="absolute inset-0 bg-gradient-to-br from-emerald-400 via-teal-400 to-emerald-600 animate-spin-slow" style={{ padding: '3px' }}>
                  <div className="w-full h-full bg-gray-950" />
                </div>
                <div className="relative w-32 h-32 overflow-hidden ring-4 ring-emerald-500/30 shadow-2xl shadow-emerald-500/20">
                  <img src="https://avatars.githubusercontent.com/VoidX3D" alt="Sincere Bhattarai" width="128" height="128" className="w-full h-full object-cover rounded-lg" loading="lazy" decoding="async" />
              ...
              <h1 className="text-5xl sm:text-7xl font-display font-extrabold text-white mb-3"><GlitchText text="Sincere Bhattarai" /></h1>
              <div className="h-8">
                <p className="text-xl sm:text-2xl text-emerald-400 font-mono"><TypewriterText text="@VoidX3D • Student • Developer • Environmental Advocate" delay={800} /></p>
              </div>
              ...
              <div className="relative mt-16 h-20">
                <div className="absolute inset-0 flex items-center justify-center gap-4 sm:gap-6">
                  {techIcons.map((icon, i) => (
                    <span key={icon.name} className="text-xl sm:text-2xl opacity-30 hover:opacity-100 transition-all duration-500 cursor-default hover:scale-125"
                      style={{ animation: `float 3s ease-in-out ${i * 0.3}s infinite`, filter: 'grayscale(0.5)' }} title={icon.name}>{icon.emoji}</span>
                  ))}
                </div>
              </div>
              ...
              {timeline.map((item, i) => (
                <div key={item.year} className={`relative flex items-start gap-6 mb-12 sm:mb-16 ${i % 2 === 0 ? 'sm:flex-row' : 'sm:flex-row-reverse'}`}>
                  ...
                  <div className="shrink-0 relative z-10">
                    <div className="w-10 h-10 bg-gray-900 border-2 border-emerald-500/50 flex items-center justify-center shadow-lg shadow-emerald-500/10">
              ...
              {connectLinks.map((link) => {
                ...
                return (
                  <a key={link.platform} href={link.url} {...(isExternal ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="ml-auto text-gray-600 group-hover:text-emerald-400 transition-colors shrink-0 relative z-10"><path d="M7 17l9.2-9.2M17 17V7H7" /></svg>
                  </a>
                )
              })}
            </div>
