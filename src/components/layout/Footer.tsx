...
export default function Footer() {
  ...
  const quickLinks = config?.footerQuickLinks?.filter(l => !['/privacy', '/consent', '/license'].includes(l.href)) || []
  const legalLinks = config?.footerQuickLinks?.filter(l => ['/privacy', '/consent', '/license'].includes(l.href)) || []
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          <div className="sm:col-span-2 lg:col-span-2">
            <Link to="/" className="flex items-center gap-3 mb-4">
              <img
                src={storageUrl(config?.logoIcon || '/static/assets/brand/logo_icon.png')}
          ...
          <div>
            ...
            <div className="text-sm text-text-secondary dark:text-dark-text-secondary space-y-1.5">
              {location && (
                <>
                  <p>{location.school}</p>
                  <p>{location.city} - {location.ward}, {location.district}</p>
        ...
        <div className="border-t border-border dark:border-dark-border mt-10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-text-muted dark:text-dark-text-muted">
            Made with care by <Link to="/secret-garden" className="text-brand-500 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">{config?.madeBy || 'Sincere Bhattarai'}</Link>
          </p>
        </div>
      </div>
    </footer>
