      ...
      <div className="flex items-start justify-between relative z-10">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            {project.tags.map((t) => (
              <span key={t} className="text-[10px] px-2 py-0.5 bg-white/5 text-gray-500 border border-white/5 rounded-lg">{t}</span>
            ))}
          </div>
        </div>
        <div className="flex gap-2 shrink-0 ml-4">
          {project.url !== '#' && (
            <a href={project.url} target="_blank" rel="noopener noreferrer"
              className="w-9 h-9 bg-white/5 flex items-center justify-center text-gray-400 hover:text-emerald-400 hover:bg-emerald-500/20 transition-all border border-white/5 hover:border-emerald-500/30 rounded-lg">
